...
class DAVISEvaluation(object):
    def __init__(self, davis_root, task, gt_set, DBtype, sequences='all', codalab=False):
        """
        ...
        self.task = task
        self.dataset = DAVIS(root=davis_root, DBtype=DBtype, task=task, subset=gt_set, sequences=sequences, codalab=codalab)
        self.types = DBtype
        ...
        for seq in (list(self.dataset.get_sequences())):
            ...
            if self.types == '2016':
                ...
                for i in range(all_gt_masks.shape[0]):
                    all_gt_masks_new = all_gt_masks[i] | all_gt_masks_new
                all_gt_masks = all_gt_masks_new[np.newaxis]

            if self.task == 'unsupervised':
                j_metrics_res, f_metrics_res = self._evaluate_unsupervised(all_gt_masks, all_res_masks, all_void_masks,
                                                                           metric)
