...
def get_model_parameters_number(model, as_string=True):
    ...
    #     print(p.shape)
    #     params_num += p.numel()
    #     print("num of accumulate param num : {}".format(params_num))
...
def compute_average_flops_cost(self):
    ...
    for module in self.modules():
        if is_supported_instance(module):
            ...
            # print(str(module) + " :  "  + "\t total: " + str(0.5*flops_sum/10**9) )
...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    # print("{} \t input : {} \toutput :{}".format(conv_module, str(input.shape), str(output[0].shape)))
...
def deconv_flops_counter_hook(conv_module, input, output):
    ...
    active_elements_count = batch_size * input.shape[2] * input.shape[3]

    if conv_module.__mask__ is not None:
        # (b, 1, h, w)

        flops_mask = conv_module.__mask__.expand(batch_size, 1,  input.shape[2], input.shape[3])
        active_elements_count = flops_mask.sum()

    overall_conv_flops = active_elements_count*conv_per_position_flops
